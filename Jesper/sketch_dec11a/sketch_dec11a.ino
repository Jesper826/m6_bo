...
void loop() {
  ...
  int effectLevel = map(rawEffect, 0, 1023, 0, totaalVolumeLEDs);
 
  for (int i = 0; i < totaalVolumeLEDs; i++) {
    digitalWrite(volumeLeds[i], i < effectLevel ? HIGH : LOW);
  }
  ...
  if (tokkelNiveau > 0) {
    int baseFreq = 260 + pitchOffset;   // VEEL lager dan 500 Hz
